 ...
 */
export function getVercelGlobalDir(
  env: NodeJS.ProcessEnv = process.env,
): string {
  if (env.VCX_VERCEL_GLOBAL_DIR) return resolve(env.VCX_VERCEL_GLOBAL_DIR);
  const home = homedir();
  let dataDir: string;
 ...
 */
export async function linkVercelGlobalDir(
  profileDir: string,
  env: NodeJS.ProcessEnv = process.env,
): Promise<LinkResult> {
...
export async function unlinkVercelGlobalDir(
  env: NodeJS.ProcessEnv = process.env,
...
async function backupPath(globalDir: string): Promise<string> {
  const base = `${globalDir}.before-vcx`;
  if (!(await lstatOrUndefined(base))) return base;
